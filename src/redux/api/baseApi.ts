...
const baseQuery = fetchBaseQuery({
  baseUrl: "http://prostuti-app-teacher-admin-dashb-staging.up.railway.app/api/v1",
  credentials: "include",
...
const baseQueryWithRefreshToken: typeof baseQuery = async (
  ...
  if (result.error && (result.error as FetchBaseQueryError).status === 401) {
    const res = await fetch("http://prostuti-app-teacher-admin-dashb-staging.up.railway.app/api/v1/auth/refresh-token", {
      method: "POST",
      credentials: "include",
    });
